...
def createWaypointStampedTransform(wp,wp_tform):
    ...
    stampedTMsg.transform.rotation.x = wp.waypoint_tform_ko.rotation.x 
    stampedTMsg.transform.rotation.y = wp.waypoint_tform_ko.rotation.y  
    stampedTMsg.transform.rotation.z = wp.waypoint_tform_ko.rotation.z  
    stampedTMsg.transform.rotation.w = wp.waypoint_tform_ko.rotation.w 
...
def createFudicialStampedTransformQuat(wo,wo_tform,quat):
    stampedTMsg = TransformStamped()
...
def createFudicialStampedTransformQuatPos(wo,pos,quat):
    stampedTMsg = TransformStamped()
...
def create_graph_objects(current_graph, current_waypoint_snapshots,current_anchors,current_anchored_world_objects, current_waypoints, renderer,pub):
    """
    ...
    while len(queue) > 0:
        ...
        print("e")
        print(mat_to_vtk(curr_element[1]).GetPosition())
        waypoint_objects[curr_waypoint.id].SetUserTransform(mat_to_vtk(curr_element[1]))
        world_tform_current_waypoint = curr_element[1]
        
        #my shtuff
        #tfmsg = createWaypointStampedTransform(curr_waypoint,waypoint_objects[curr_waypoint.id].GetUserTransform())
        anchor = current_anchors[curr_waypoint.id]
        tfmsg = createWaypointStampedTransformQuatPos(curr_waypoint,anchor.seed_tform_waypoint.position,anchor.seed_tform_waypoint.rotation)
        ...
        if (curr_waypoint.snapshot_id in current_waypoint_snapshots):
            ...
            for fiducial in snapshot.objects:
                if fiducial.HasField("apriltag_properties"):
                    ...
                    msg = createFudicialStampedTransform(fiducial,mat_to_vtk(world_tform_fiducial))
                    ...
                    for anchor_key in current_anchored_world_objects:
                        for anchor in current_anchored_world_objects[anchor_key]:
                            if anchor.id == '520':
                                ...
                                msg = createFudicialStampedTransformQuat(fiducial,mat_to_vtk(world_tform_fiducial),tform.rotation)
                                pub.publish(msg)
...
def main(argv):
    ...
    while not rospy.is_shutdown():
        ...
        create_graph_objects(current_graph, current_waypoint_snapshots,current_anchors,current_anchored_world_objects, current_waypoints,
                                        renderer,pub)
        loop.sleep()
